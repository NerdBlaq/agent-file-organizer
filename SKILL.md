...
**Some directories are never individually sorted, in any mode.** A directory containing a `.exe`/`.dll`/`.so`/`.dylib`/`.bin`/`.msi` is treated as an atomic software or game package — the scanner never reaches inside it, even under `reorganize` mode, which otherwise ignores existing folder boundaries entirely. This exists because reorganize mode flattening a curated document folder is a misfiling (annoying, fully recoverable); the same behavior against a game or application install actually breaks the software, which is a different order of harm that no taxonomy override should be able to touch.
...
Report a concise, concrete summary: how many files moved to each destination, how many were left unsorted, skipped items, any protected package directories left untouched, and the undo log path.
