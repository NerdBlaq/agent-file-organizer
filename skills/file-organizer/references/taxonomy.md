...
## Special case: software/game package directories (v1.1.2, extended v1.1.3, Integrity Guard)

A directory is treated as an atomic package — never reached into, in every structure mode including `reorganize` — if either of two independent signals fires:

1. **Content-based**: any file anywhere in its subtree (checked recursively, not just immediate children) has a `.exe`, `.dll`, `.so`, `.dylib`, `.bin`, or `.msi` extension.
2. **Name-based (v1.1.3)**: the directory's own name matches a known curated-content pattern — `games`, `my games`, `saved games`, `.minecraft`, `shaderpacks`/`shader packs`, `resourcepacks`/`resource packs`, `mc extras`, `mc shaders`, `mc mods`, `modpacks`, `steamapps` — case-insensitive substring match.

The name-based signal exists because the content-based check alone misses most real cases: a folder of compressed game archives (`.rar`/`.zip`, no visible `.exe`), a save-data folder (configs, `.dat`, `.plr`, `.wld` — no binaries at all), or a shader/resource pack (`.zip`/`.json` — no binaries). All three were confirmed torn apart in real incident logs specifically because nothing in them matched the content-based check.

Extend the name list per-install via `protected_folder_names` in config — this is also the mechanism for "never touch this specific folder no matter what," e.g. a personal reference folder that's already organized exactly how the user wants: add its name there, or use `exclude` in config for an exact path rather than a name pattern (see `assets/config.example.yaml`).

Reorganize mode turns this protection off, which means files that are already inside an organized folder become eligible to move again under the plain taxonomy rules above — including cases where the folder was carrying information a tag or filename doesn't (e.g. an artist folder with no ID3 tags). Don't treat Reorganize as "the thorough version of Extend" — it's a different, higher-risk operation that can throw away information the user's own filing already captured, and it needs its own explicit confirmation, not a rollover from a general "looks good."

## Special case: self-nesting (v1.1.3)

If a file is already sitting directly inside a folder named e.g. "Documents", and its classified destination bucket is also named "Documents," the file is left exactly where it is rather than being nested into a "Documents/Documents" duplicate. Confirmed as a real, repeated failure — scanning a directory whose own name coincides with a taxonomy bucket name produced this nesting in two separate incident logs. This check only fires when the names match but the paths differ (i.e., it's about to create a genuinely redundant same-named child); it does not suppress an intentional destination that happens to share a name with something unrelated elsewhere in the tree.
