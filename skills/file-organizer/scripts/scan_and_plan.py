...
# v1.1.3: content-based check above only catches an extracted install with
# visible binaries. Confirmed against real incident logs, it misses a
# folder of compressed game archives (.rar/.zip with no visible .exe),
# save-data folders (configs/.dat/.plr/.wld, no binaries at all), and
# resource/shader packs (.zip/.json, no binaries). Folder name is the only
# reliable signal for these. Extend via config's `protected_folder_names`.
PROTECTED_FOLDER_NAME_PATTERNS = {
    "my games", "saved games", ".minecraft", "shaderpacks", "shader packs",
    "resourcepacks", "resource packs", "mc extras", "mc shaders", "mc mods",
    "modpacks", "steamapps", "games",
}


def looks_like_package_dir(dir_path: Path, extra_name_patterns=None) -> bool:
    """v1.1.2 Integrity Guard, extended in v1.1.3 with folder-name detection
    (see PROTECTED_FOLDER_NAME_PATTERNS) and recursive content scanning
    (a binary a few folders deep, e.g. Game/bin/game.exe, still protects
    the whole Game/ folder, not just bin/)."""
...
def walk_target(target: Path, recursive: bool, exclude_set, structure_mode: str, recognized: set, protected_dirs: list = None, extra_protected_names=None):
    ...
    for root, dirs, files in os.walk(target):
        ...
        if looks_like_package_dir(root_path, extra_protected_names) and root_path != target:
            # Never applied to the top-level scanned target itself — see
            # v1.1.3 note in the package version for why.
