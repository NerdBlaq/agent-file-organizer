...
def main():
    ...
    with open(log_path, "a", buffering=1) as log_f:
        for m in plan.get("moves", []):
            ...
            # v1.1.3: self-nesting guard, checked before creating dest_dir.
            # Confirmed against two real incident logs — scanning a folder
            # literally named "Documents" whose own files classify into a
            # "Documents" bucket produces .../Documents/Documents/file.odt.
            if dest_dir.name.lower() == src.parent.name.lower() and dest_dir.resolve() != src.parent.resolve():
                print(f"SKIP (self-nesting, already inside a '{src.parent.name}' folder): {src}", file=sys.stderr)
                skipped_noop += 1
                continue
