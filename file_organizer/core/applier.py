...
def apply_plan(
    ...
    with open(log_file, "a", buffering=1) as log_f:
        for m in plan.get("moves", []):
            ...
            # v1.1.3: self-nesting guard, checked BEFORE creating dest_dir so
            # we don't even create the empty nested folder. Confirmed against
            # two separate real incident logs — scanning a folder named e.g.
            # "Documents" whose own resident files get classified into a
            # bucket also named "Documents" produces
            # .../Documents/Documents/file.odt. If the destination folder's
            # name matches the name of the folder the file is already
            # sitting in, the file is already "in a Documents-shaped place"
            # — don't nest a duplicate.
            if dest_dir.name.lower() == src.parent.name.lower() and dest_dir.resolve() != src.parent.resolve():
                skipped_noop += 1
                actions.append({"src": str(src), "status": "skipped_self_nesting",
                                 "note": f"already inside a '{src.parent.name}' folder; not nesting another one"})
                continue
