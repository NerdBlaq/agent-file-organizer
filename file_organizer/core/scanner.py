...
from file_organizer.taxonomy import (
    ...
    PACKAGE_EXTS,
    PROTECTED_FOLDER_NAME_PATTERNS,
)
...
def looks_like_package_dir(dir_path: Path, extra_name_patterns=None) -> bool:
    """v1.1.2 Integrity Guard, extended in v1.1.3.

    Two independent triggers, either one is sufficient:
    1. Name-based: the directory's own basename matches a known
       game/save/pack pattern (case-insensitive substring), builtin plus
       any config-supplied `protected_folder_names`. This is the trigger
       that actually catches real-world cases confirmed in incident logs
       — a "GAMES" folder of .rar archives, a "My Games/Terraria" save
       folder, an "MC Shaders" folder of .zip packs — none of which
       contain a binary file anywhere for content-based detection to find.
    2. Content-based: any file *anywhere in the subtree* (checked
       recursively, not just immediate children — a game whose binary
       lives a few folders deep, e.g. Game/bin/game.exe, still protects
       the whole Game/ folder) matches PACKAGE_EXTS.
    """
    name_lower = dir_path.name.lower()
    patterns = PROTECTED_FOLDER_NAME_PATTERNS | set(extra_name_patterns or [])
...
def walk_target(target: Path, recursive: bool, exclude_set: List[str], structure_mode: Optional[str], recognized: Set[str], protected_dirs: Optional[List[str]] = None, extra_protected_names=None):
    ...
    for root, dirs, files in os.walk(target):
        ...
        if any(str(root_path).startswith(str(Path(e).expanduser())) for e in exclude_set):
            dirs[:] = []
            continue
        if looks_like_package_dir(root_path, extra_protected_names) and root_path != target:
            # Never apply this to the top-level scanned target itself —
            # the user explicitly chose to organize inside it. Recursive
            # content scanning means a target that merely *contains* a
            # game folder somewhere within it would otherwise flag the
            # whole target as protected, which is wrong; protection is
            # for subdirectories the walk encounters, not the root the
            # user asked to organize.
            # v1.1.2 Integrity Guard: this overrides structure_mode entirely,
            # including "reorganize" — a software/game install being torn
            # apart isn't a filing preference to respect or override, it's
            # damage regardless of which taxonomy mode asked for it.
...
def build_plan(
    ...
    extra_protected_names = [n.lower() for n in (config.get("protected_folder_names", []) or [])]
    ...
    for target in all_targets:
        ...
        for path in walk_target(target, effective_recursive, exclude_set, target_mode, recognized, protected_dirs, extra_protected_names):
...
def render_plan_markdown(plan: Dict[str, Any]) -> str:
    ...
    if plan.get("protected_dirs"):
        md_lines.append(f"⚠️ {len(plan['protected_dirs'])} directory/directories protected as software/game packages (contain .exe/.dll/.so/.dylib/.bin/.msi) — left untouched:")
        for pd in plan["protected_dirs"]:
