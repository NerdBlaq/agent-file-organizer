...
# v1.1.3: the check above only catches an *extracted* install with visible
# binaries. Confirmed against real incident logs, it misses the more common
# case in practice — a folder of compressed game archives (.rar/.zip with
# no visible .exe), save-data folders (configs/.dat/.plr/.wld, no binaries
# at all), and resource/shader packs (.zip/.json, no binaries). Folder
# *name* is the only reliable signal for these, checked independently, not
# as a fallback. Matched case-insensitively as a substring of the
# directory's own basename. Extend via config's `protected_folder_names`
# rather than editing this constant per-install.
