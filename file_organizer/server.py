...
def create_mcp_server():
    ...
    def log_session_event(
        ...
        """Record a structured entry about this run — what the user asked for, what mode
        was chosen, and critically what they actually said to confirm an action. This is
        separate from the mechanical moves log (which only records src->dest) and exists
        because every incident review of this tool has had to be reconstructed by hand
        from chat transcripts. Call this at the start of a run and again after any
        confirmation or apply step — it has no access to the conversation itself, so the
        calling agent has to supply the context.

        Args:
            event: short label — "run_started", "mode_chosen", "confirmed", "applied", "recovery", etc.
            user_request: the user's own request, verbatim or faithfully summarized
            mode: quick / guided / autonomous
            structure_mode: extend / reorganize / fresh-only
            confirmation: what the user actually said to approve this action, verbatim where practical
            notes: anything else worth recording
        """
        path = log_session(event=event, user_request=user_request, mode=mode,
                            structure_mode=structure_mode, confirmation=confirmation, notes=notes)
        return json.dumps({"logged_to": str(path)})

    @mcp.tool()
    def get_session_log() -> str:
        """Return the recorded session log — what was asked, chosen, and confirmed across
        past runs. Useful for reconstructing what happened before answering a question
        about a past run instead of guessing from the moves log alone."""
        return json.dumps(read_sessions(), indent=2)
                ...
                resp = {
                    ...
                    "result": {
                        "tools": [
                            ...
                            {"name": "log_session_event", "description": "Record what was asked, chosen, and confirmed for this run."},
                            {"name": "get_session_log", "description": "Return the recorded session log."},
                        ]
                    }
                }
            elif method == "ping":
